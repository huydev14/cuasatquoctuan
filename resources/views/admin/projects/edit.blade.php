...
@section('title', 'Sửa Công trình')

@section('header')
    <div class="flex items-center justify-between w-full">
        <div>
            <h2 class="text-2xl font-bold">Sửa Công trình</h2>
            <p class="text-sm text-slate-500">Cập nhật thông tin công trình</p>
        </div>
        <a href="{{ route('admin.projects.index') }}" class="text-sky-500 hover:text-sky-700 font-semibold">← Quay lại</a>
    </div>
@endsection
    ...
    <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data"
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="6"
                class="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500/20">{{ old('description', $project->description) }}</textarea>
        </div>
        ...
        <label class="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
            <input type="checkbox" name="status" value="1" @checked(old('status', $project->status))
                class="h-4 w-4 rounded border-slate-300 text-sky-500 focus:ring-sky-500">
            <span class="text-sm font-semibold text-slate-700">Hiển thị công trình</span>
        </label>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.projects.index') }}" class="text-slate-600 hover:text-slate-900">Hủy</a>
        </div>
    </form>
    </div>
@endsection
